...
function VocabTestContent() {
  ...
  const { targetLanguage, setVocabLevel } = useLandingV3()
  ...
  const wordLists = targetLanguage === 'French' ? FR_WORDS : PT_WORDS
  ...
  const handleDontKnow = () => {
    ...
    setTimeout(() => {
      router.push('/landing-v3/setup/analyzing')
    }, 2000)
  }
  ...
  const handleKnowAll = () => {
    if (nextLevel) {
      // Go to next level
      router.push(`/landing-v3/setup/vocab-test?level=${nextLevel}`)
    } else {
